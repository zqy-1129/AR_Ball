...
- **引导是分层的**：球面 8 片目标面琥珀高亮 + 环上 8 段进度 + 中央一句方向指令（「向右转」），
  眼睛不用离开物体也知道下一步往哪走；每点亮一个面手机震一下。
- **稳定 60 FPS**：单帧 3 次 draw call，绘制耗时 0.07 ~ 0.2 ms（模拟器）/ 0.2 ~ 1.4 ms（真机）。
- 无第三方 UI 依赖，全部 Kotlin + OpenGL ES 3.0 + Canvas 自绘，12 个文件共约 4000 行。
...
| 控件 | 位置 | 作用 |
| --- | --- | --- |
| **快门** | 底部正中 | 手动模式下按一下 = 拍下当前正对的那个面；自动采样下也可按，等于「现在就拍这一面」 |
| **自动采样 / 手动快门** | 底部左 | 切换模式。自动采样：对准即整面点亮；手动：只有按快门才记录 |
| **重置** | 底部右 | 清空全部覆盖数据。**点两次才生效**：第一次变成琥珀色的「确认重置」，2.5 s 内不确认自动撤防 |
| **自动巡航** | 右上第三行下方 | 一键启动/停止绕物体水平环绕（约 16 s 一圈），没有传感器的设备也能演示 |
| **8 段覆盖度环** | 套在球外 | 一段 = 一个面，完成一段亮一段（青绿）；外圈那段更细的弧 = 当前面已对准的进度 |
| **琥珀箭头** | 环外 | 指向目标面在屏幕上的方向；接近时自动淡出 |
| **中央文字** | 画面中部 | 按状态切换：`缓慢环绕物体` → `向右转 / 向左转 / 抬高手机 / 压低手机` → `已对准 · 保持不动` → `采集完成` |
| **完成横幅** | 顶部 | 8 个面全满时弹出「360° 无死角覆盖完成」，**整块可点 = 重新扫描** |
| **首次引导** | 全屏 | 首次安装启动时出现，三页图文说明；点哪都翻页，右上角「跳过」直达 |

> 球体面板右下角那行状态文字会按情况切换：
> `待授权 / 无相机 / 全部覆盖 ✓ / 采集中 N% / 向右转 / 还差 N 个面`。

**触觉反馈**：举手绕拍时用户多半盯着真实物体而不是屏幕，所以关键节点都有震动 ——
按钮是轻点，**每点亮一个面**是中震（不看屏幕也知道拍到了），8 个面全满是一段庆祝节奏。
没有振动马达的设备（部分模拟器）会自动静默跳过。
...
0. **首次安装启动会先看到三页引导**（把物体放进取景框 / 绕着物体转一圈 / 8 个面全亮就完成）。
   点哪都翻页，右上角「跳过」可直达。引导期间**采集是冻结的**，所以看完之后的进度
   是从真正的 `0%` 开始，而不是「一打开就已经拍了 1/8」。
...
3. **以物体为中心，缓慢水平转一圈**（人绕着物体走，或人站着慢慢转身）。
   镜头停到某个面上，中央会提示 `已对准 · 保持不动`，同时环外那段细弧开始填满；
   约 `0.22 s` 后该面 21 个点整面亮起、手机震一下、覆盖度按 `+1/8` 跳。
4. 跟着**中央的方向指令**和**环外箭头**走：写「向右转」就往右转、写「抬高手机」就抬高，
   转过 45° 就换下一个面。提示语会自己换成「保持不动」告诉你已经对准了。
5. 左上角到 **`100%`（已拍 8/8 个面）** 时，顶部会弹出完成横幅 —— 点它即可开始下一轮，
   不必回底部找那个要二次确认的「重置」。
...
| 自动采样停留时长 | `ScanSession.FACE_DWELL_SECONDS`（默认 `0.22 s`） | 调小更容易「路过即拍」，调大更稳但更慢；调大后环外那段对准弧也会更明显 |
...
| 方向指令的触发阈值 | `HudView.dirWord()` 里的 `t = 0.10f` | 目标离光轴多近就不再催、改说「保持不动」 |
| 震动强度 / 节奏 | `ui/Haptics.kt` 的 `tick / faceCaptured / allDone` | 没有马达的设备自动静默，无需额外判断 |
| 「重置」二次确认窗口 | `HudView.RESET_ARM_MS`（默认 `2500 ms`） | 上膛后多久自动撤防 |
| 首次引导文案 | `HudView.ONBOARDING_TITLES / ONBOARDING_BODIES` | 三页文案与页数；改页数要同时改 `ONBOARDING_*` 两个数组长度 |
| 重新触发首次引导 | 清数据 `adb shell pm clear com.remy.guidesphere` | 引导只在 `SharedPreferences` 里没有 `onboarding_done` 时出现 |
...
| `cam` | 相机此刻正对着的扇区（0~7） |
| `target` | 当前建议采集的目标扇区（0~7，顺时针推进；全部完成时停在最后一个） |
| `dwell` | **镜头正对的那个面已经对准了多久**（0~100%）。到 100% 整面点亮 —— 用它就能确认「对准即拍」在工作 |
...
│               └── ui/
│                   ├── HudView.kt               纯 Canvas 自绘 HUD（含首次引导）
│                   └── Haptics.kt               震动反馈（无马达设备自动静默）
...
> **首次安装**不会遇到这个现象：第一次启动会先显示三页引导，引导期间采集被冻结，
> 所以看完之后是从真正的 `0%` 开始。之后每次启动都是直接进扫描界面，第一个面
> 会在 `0.22 s` 内自动记上 —— 演示视频因此从 `25%（2/8）` 起而不是 0%。

**Q：为什么「重置」点了没反应？**
它需要**连点两次**：第一次会变成琥珀色的「确认重置」，2.5 s 内再点一次才真的清空。
这是故意的 —— 扫到一半误触清空，比多点一下难受得多。

**Q：怎么再看一次首次引导？**
清掉应用数据即可：`adb shell pm clear com.remy.guidesphere`（注意会同时撤销相机授权，
需要重新授予）。引导的完成标记存在 `SharedPreferences` 的 `onboarding_done` 里。
...
| `deliverables/guidesphere_demo.mp4` | 演示录屏：三页首次引导 → 绕物体水平一圈，8 个面依次整面点亮 → 100% 完成横幅 |
| `deliverables/ux_01_onboarding.png` ~ `ux_03_onboarding.png` | 首次引导三页 |
| `deliverables/ux_04_aiming.png` | 「已对准 · 保持不动 / 正在采集这一面 98%」+ 环外对准进度弧 |
| `deliverables/ux_05_scanning.png` | 扫描中：8 段环已亮 3 段 + 目标段琥珀呼吸 + 「向右转」指令 |
| `deliverables/ux_06_completed.png` | 完成态：8 段全亮 + 完成横幅 + 「点按此处重新扫描」 |
| `deliverables/guidesphere_gray_sphere.png` | 截图：灰色球壳 + 8 面点阵 + 目标面琥珀高亮 |
